...
func (rpl *REPLICA) WatchCtn()  {
	var exitWatchCtnChan chan int
	exitWatchCtnChan = make(chan int,1)
	var statusMap map[string]int
	statusMap = make(map[string]int,1)
	pool.RegPrivateChanStr(rpl.CtnName)
	for {
		select {
