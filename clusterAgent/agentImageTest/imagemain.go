...
func imagemain() {
	...
	// //3 history image
	// imageHistory, isSuccess := agentimage.HistoryImage(imgid)
	// if !isSuccess {
	// 	//deal error
	// 	return
	// }
	...
	// isSuccess = agentimage.TagImage(sourceName, targetName)
	// if !isSuccess {
	// 	//deal error
	// 	return
	// }
...
func containermain() {
	...
	containerapi.ContainerInit(ctx, cli)

	//获取运行中的容器列表
	_, isSuccess := containerapi.ListofContainer(true)
	if !isSuccess {
		return
	}
	...
	containerID := containerapi.ContainerNameMap[containerName]
	// log.Println("containerapi.ContainerNameMap", containerapi.ContainerNameMap)
	...
	//update container
	containerapi.UpdateContainerMap(containerName, resp.ID)
	...
	// //容器日志
	// logstr, isSuccess := containerapi.LogsofContainer("af23c7ffaad9", true, true, "10m", "", true, "10")
	// if !isSuccess {
	// 	return
	// }
...
func DrawContainerPLot(containerName string) {
	...
	for t := range ticker.C {
		...
		stasstr, isSuccess := containerapi.StatsofContainer(containerID[:10], false)
		if !isSuccess {
			return
		}
		...
		drawapi.DrawPlot("container-cpu", "time", "cpu%", curValue)
	}
}
...
func DrawAllContainerStats() {
	...
	//获取运行中的容器列表
	containers, isSuccess := containerapi.ListofContainer(false)
	if !isSuccess {
		return
	}
	...
	for _, container := range containers {
		stasstr, isSuccess := containerapi.StatsofContainer(container.ID[:10], false)
		if !isSuccess {
			return
		}
		splitstr := strings.Split(stasstr, " ")
	...
	drawapi.DrawBar( /*splitstr[0]+" "+splitstr[1]*/ "cpuVSmem image", names, cpudatas, memdatas)
